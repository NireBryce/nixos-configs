...
> **Written by Claude Code, for Claude Code.** An agent's working notes, not
...
- **`virtualization`** — libvirt/QEMU VMs. Cube is the only importer; durandal
  ...
  Cube runs `nire-llm-sandbox` through it, generated by `VMs/_lib/libvirt-vm.nix`
  (see skill `nixos-vm-images`): that generator starts libvirt's default NAT
  network itself when a VM asks for one (libvirt ships it defined-but-stopped),
  and takes an `sshForward` parameter restricted by source IP — the sandbox
  uses it, tailnet-only, port 2222. Its first outing hit three runtime-only
