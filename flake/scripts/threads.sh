...
# `_loose-ends/bugs-pending-submission/` for the same term.
#
...
# wiki/ and _loose-ends/ both live at the true repo root, one level
# above flake/ -- this script sits in flake/scripts/, so that's two
...
echo
echo "=== wiki/, _loose-ends/bugs-pending-submission/ ==="
...
search_paths=(wiki)
bugs=_loose-ends/bugs-pending-submission
[[ -d "$bugs" ]] && search_paths+=("$bugs")
