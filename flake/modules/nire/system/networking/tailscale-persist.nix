...
# imports `impermanence`, true of both hosts that existed then. nire-testbed
# does not -- persistent root, nothing wipes /var/lib/tailscale -- and this
# entry would have been pure overhead for it: a same-filesystem bind mount
...
# guards now is testbed getting an entry, and the warning, for no reason.
#
...
# see that file's "OPT-IN, SINCE NIRE-TESTBED" header note for why that
# specific signal and not something else.
{ lib, ... }:
    let
