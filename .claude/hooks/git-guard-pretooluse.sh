...
# secrets-guard-pretooluse.sh exists for `sops -d`.
#
...
if [ -z "$reason" ] && grep -qE '\breset\b' <<<"$command" && grep -qE -- '--hard\b' <<<"$command"; then
    reason="'git reset --hard' discards uncommitted changes and moves the branch, losing any commits not reachable elsewhere. Confirm nothing unsaved is about to be dropped -- this is also the ship skill's documented recovery step for an accidental commit to experimental, so it's expected in that specific case."
fi
