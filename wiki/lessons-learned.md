...
2026-09-14, tenacity, reviewing the yubikey module. `security.pam.u2f.settings.authFile` — camelCase, since nixpkgs renamed the option to `settings.authfile` — had passed eval for five months: `settings` is freeform, unknown keys render verbatim into the PAM line, and pam_u2f discards unknown module arguments. Login still worked, because the explicit path equaled pam_u2f's built-in default (`$XDG_CONFIG_HOME/Yubico/u2f_keys`). No eval error, no checker — the rendered `/etc/pam.d/sudo` carrying `authFile=` was the only tell. The same review found the live system correcting two docs-shaped assumptions: u2f was already on *every* PAM service (each service's `u2fAuth` defaults to `security.pam.u2f.enable`, so the module's explicit `login`/`sudo` lines only restated a default — §33's "agrees with the default, reads as a decision"), and a drafted wake script's first-active-session rule lost to an unseated `manager` session that also reported `State=active`. §33 stops the next misspelling at write time; the rendered artifact audits the ones already shipped.

Full account: [49-freeform-settings-render-unknown-keys.md](lessons-learned/49-freeform-settings-render-unknown-keys.md).
