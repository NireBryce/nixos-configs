...
vhost except `git.` carries `vmDeny` — a `remote_ip 192.168.122.0/24`
matcher in a `handle` that aborts — so a guest sending another Host/SNI
gets nothing. It is a `handle`, not a bare `abort`, because `handle`
sorts ahead of `abort` and the landing vhost's catch-all `handle` would
win otherwise (checked with `caddy adapt`, 2.11.4).
