...
| `secrets/forgejo-api-key.nix` | The ONLY `sops.secrets.*` declared in this category. `sops.secrets.forgejo_api_key = { owner = "elly"; mode = "0400"; }` -> `/run/secrets/forgejo_api_key` on all three Linux hosts. Client credential (Forgejo REST API, run by the user/agent), not a service secret, so the "declare it where the service runs" rule that keeps `forgejo-admin-password` in `forgejo.nix` doesn't apply. Cube renders it too — accepted, all hosts can decrypt the file anyway. Underscores in the key name are load-bearing (sops-nix derives `.key` from the attribute name; wrong spelling fails at ACTIVATION, not eval). |
