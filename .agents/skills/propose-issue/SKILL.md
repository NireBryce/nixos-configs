...
4. **On yes:**
   ...
   obvious. End with `🤖 Generated by an LLM agent` — deliberately that,
   not the PR flow's Claude Code badge: an agent can't verify which model is
   executing it, the same reasoning behind `CLAUDE.md`'s agent-name-only
   commit trailer, so the line claims only what it knows. Report the issue
   URL back in your reply.
