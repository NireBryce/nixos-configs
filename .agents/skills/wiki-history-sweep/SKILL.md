...
- **A `-history.md` page needs no `-for-agents` sibling, but it DOES keep a
  ...
  An earlier version of this skill got it backwards and
  `reverse-proxy-history.md` lost its block on 2026-09-12 as a direct
  result. `check_contents` now reports a **MISSING CONTENTS** finding for a
  non-exempt page that has headings but no block, so the same mistake fails
